...
(function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        paginationLoop: false, //2018.07.17 分页循环禁用
    ...
    BootstrapTable.prototype.initContainer = function () {
        ...
        if (this.options.addTopBorder) {
            this.$tableContainer.css("border-top", "1px solid #ddd");
        } else {
            this.$tableContainer.css("border-top", "");
        }
    };
    ...
    BootstrapTable.prototype.initServer = function (silent, query, url) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function (res) {
                //TODO:2017-09-01新加的代码,处理页码错误问题开始，需注意分页查询返回的数据格式 res中的total rows
                if (that.options.pagination && res && res.total && !res.rows.length && res.total > 0 && res.rows.length == 0) { //总记录数大于0,但当前页记录数为0,则此时页码超过了最大页码误
                    that.options.pageNumber = Math.ceil(res.total / that.options.pageSize);//最后一页(总页数)
                    that.initServer();
                    return;
                }
                if (!!res.isFail) {
                    if (check_login_after_pop_req(res)) {
                        showRespFail(res.msg);
                    }
                }
                //TODO END
            ...
            error: function (res) {
                //TODO:2017-09-01新加的代码
                if (res.status != 0) {
                    showRespError(res.status)
                }
                //TODO END
    ...
    var allowedMethods = [
        ...
        'toggleView',
        'refreshOptions',
        'loadAddSearch', //加入自定义搜索
        'addTopBorder', //添加表格顶部的线
        'resetSearch',
