(function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        //modify by halason on 17-12-06
        //解决多行融合行固定问题,目前只对特殊情况一行固定的进行处理
        var last_th = $tr.find('th:last');
        var rowspan = last_th.attr('rowspan');

        if (!!rowspan && rowspan >= 2 && this.options.fixedNumber == 1 || this.options.fixedNumber == 2) {
            for (var i = 0; i < (rowspan - 1); i++) {
                $tr = $tr.clone();
                $tr.html('').append('<th style="height: 81px;"></th>');
                this.$fixedHeaderColumns.append($tr);
            }
        }
        //end
    };
